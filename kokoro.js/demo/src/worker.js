...
async function initialize() {
  ...
  self.addEventListener("message", async (/** @type {MessageEvent<import("./messages.js").GenerateRequest>} */ e) => {
    if (running) {
      console.warn("Ignoring a duplicate generation request while speech is running.");
      return;
    }
    running = true;
    try {
      const { type, text, voice } = e.data;
      if (type !== "generate" || typeof text !== "string" || !text.trim() || !Object.hasOwn(tts.voices, voice)) {
        throw new Error("Enter non-empty text and select an available voice.");
      }
